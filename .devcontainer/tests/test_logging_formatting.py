...
import pytest
from app.logging_setup import log_event, current_update_id
...
async def test_log_event_injects_display(caplog):
    ...
    assert data["user_display"].startswith("#111")
    assert data["chat_display"].startswith("#222")
...
async def test_handle_message_sets_update_id_and_logs(caplog, monkeypatch):
    ...
    assert data["user_display"].startswith("#444")
    assert data["chat_display"].startswith(f"#{gid}")